...
# ID DO GRUPO (peguei do @userinfobot)
GROUP_ID = -1003819017548

# Nome da foto (coloque a imagem na mesma pasta)
...
def create_user(user_id, username, name, referred_by=None):
    ...
    c.execute("INSERT INTO users (user_id, username, name, register_date, wallet_id, referred_by, verified) VALUES (?, ?, ?, ?, ?, ?, 1)",
...
async def check_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verifica cache
    if user_id in verified_cache and verified_cache[user_id] > (datetime.now() - timedelta(minutes=5)).timestamp():
        return True
    
    try:
        chat_member = await context.bot.get_chat_member(chat_id=GROUP_ID, user_id=user_id)
        if chat_member.status in ["member", "administrator", "creator"]:
            verified_cache[user_id] = datetime.now().timestamp()
            return True
        return False
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando /start com foto e verificação de grupo"""
    ...
    if not in_group:
        keyboard = [
            [InlineKeyboardButton("📢 Entrar no Grupo", url=GROUP_LINK)],
            [InlineKeyboardButton("✅ Já entrei! Verificar", callback_data='verificar_grupo')]
        ]
        ...
        await update.message.reply_text(
            ...
            f"✅ Clique no botão abaixo para entrar no grupo e depois em 'Já entrei! Verificar'.",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='Markdown'
        )
        return
    
    # Usuário está no grupo, cria conta se não existir
...
async def verify_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Callback para verificar após entrar no grupo"""
    ...
    if in_group:
        ...
        await query.edit_message_text(
            ...
            "Você agora pode usar o bot normalmente.\n\n"
            "Clique em /start para começar."
        )
    else:
        keyboard = [[InlineKeyboardButton("📢 Entrar no Grupo", url=GROUP_LINK)]]
        await query.edit_message_text(
            "⚠️ **Você ainda não está no grupo!**\n\n"
            "Entre no grupo clicando no botão abaixo e depois clique em 'Verificar'.",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def mostrar_cartoes_por_bin(query, bin_num):
    ...
    keyboard = []
    for card in cards[:5]:
        keyboard.append([InlineKeyboardButton(f"💳 {bin_num}xxxxxx - R$ {card[3]:.2f}", callback_data=f'comprar_cartao_{card[0]}')])
...
async def ofertas_menu(query):
    ...
    for i, oferta in enumerate(ofertas[:5], 1):
        text += f"- **{oferta[1]}**\n"
        text += f"  ~~R$ {oferta[4]:.2f}~~ → R$ {oferta[3]:.2f} (-{oferta[5]}%)\n\n"
    
    text += "\n" + "\n".join([f"{i}. {o[1]} — R$ {o[3]:.2f}" for i, o in enumerate(ofertas[:5], 1)])
